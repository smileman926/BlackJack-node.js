...
//document.ontouchmove = function (e) {
  //e.preventDefault();
//}
//aspect ratio of background image is 1.76
...
else
{
  ...
  function resizeCoin(){
    ...
    var x=$('.circle_panel').offset().left+20;
    var y=$('.circle_panel').offset().top+20;
    console.log(x+':'+y);  
    for(var i=0;i<$elements.length;i++){
      $elements.eq(i).css('left',x++).css('top',y--);
    }
  }
  function resizeCoinAnimation(){
    console.log('Coin Resized');
    $elements=$('body').children('.chipBtn');
    $('body').children('.chipBtn').removeClass('hidden_chip');
    var x=$('.circle_panel').offset().left+20;
    var y=$('.circle_panel').offset().top+20;
    console.log(x+':'+y);  
    for(var i=0;i<$elements.length;i++){
      $elements.eq(i).animate({left:x++,top:y--});
    }
  }
...
var playerN=$('.user_name_list').html();
let deck = [];
var waitingTime=30000;
var globalTimeout1;
console.log(money);
let playerWallet = money || 0;
let playerName = playerN || 'player';
let player = {name: playerName, hand: [], money: playerWallet,id:id, bet: 0, splitHand: null};
...
function setUpTable () {
  ...
  let $quitButton = $('<div>', {'id': 'quit-button', 'class': 'menu-button'}).html('<a href="/logout"><i class="fa fa-sign-out" aria-hidden="true"></i></a>');
  ...
  let $chipsArea=$('<div>',{'id':'chips-area','class':'chips-content'});
  let $counttimer=$('<p>',{'class':'counttimer_down','style':'visibility:hidden'}); 
  $counttimer.append('Secs Remaing');
  ...
  $('.moneyStatus').append($moneyBox);
  $('#money-box').append($playerMoney);
  $('#money-box').append($playerBet);


  $('.moneyStatus').append($totalBox);
  ...
  $('#chips-area').append($fiftychip);
  $('#chips-area').append($fivehchip);
  $('#chips-area').addClass('remove_important');
 
    console.log($('#player-hand'));
  $('#card-table').append($counttimer);
...
function placeBet(turn) {
  ...
  // });
   ...
   countTimerout=setInterval(function(){
        countTime--;
        $('.counttimer_down').css('visibility','initial');
        $('.counttimer_down').html(countTime+' Secs Remaining');    
     },1000);
...
countTimerout=setInterval(function(){
  ...
  countTime--;
  $('.counttimer_down').css('visibility','initial');
  $('.counttimer_down').html(countTime+' Secs Remainig');    
},1000);
...
function dealCards(players, dealer) {
  ...
  // for (let i = 1; i <= player.hand.length; i++) {
    setTimeout(function() {
      $(`#player-hand div:nth-child(1)`).removeClass('hidden');
      $(`#player-hand div:nth-child(1)`).addClass('flyin');
      if (playSound === true) {
        playCardSound('slide');
      }
    }, timeout);
  ...
  players.forEach((otherPlayer) => {
    if (otherPlayer.id !== socket.id) {
      // for (let i = 1; i <= otherPlayer.hand.length; i++) {
        setTimeout(function() {
          $(`#${otherPlayer.id} div:nth-child(1)`).removeClass('hidden');
          $(`#${otherPlayer.id} div:nth-child(1)`).addClass('flyin');
          if (playSound === true) {
            playCardSound('slide');
          }
        }, timeout);
  ...
  // for (let i = 1; i <= dealer.length; i++) {
    setTimeout(function() {
      $(`#dealer-hand div:nth-child(1)`).removeClass('hidden');
      $(`#dealer-hand div:nth-child(1)`).addClass('flyin');
      if (playSound === true) {
        playCardSound('slide');
      }
    }, timeout);
  ...
  // };
   // for (let i = 1; i <= player.hand.length; i++) {
    setTimeout(function() {
      $(`#player-hand div:nth-child(2)`).removeClass('hidden');
      $(`#player-hand div:nth-child(2)`).addClass('flyin');
      if (playSound === true) {
        playCardSound('slide');
      }
    }, timeout);
  ...
  players.forEach((otherPlayer) => {
    if (otherPlayer.id !== socket.id) {
      // for (let i = 1; i <= otherPlayer.hand.length; i++) {
        setTimeout(function() {
          $(`#${otherPlayer.id} div:nth-child(2)`).removeClass('hidden');
          $(`#${otherPlayer.id} div:nth-child(2)`).addClass('flyin');
          if (playSound === true) {
            playCardSound('slide');
          }
        }, timeout);
  ...
  // for (let i = 1; i <= dealer.length; i++) {
    setTimeout(function() {
      $(`#dealer-hand div:nth-child(2)`).removeClass('hidden');
      $(`#dealer-hand div:nth-child(2)`).addClass('flyin');
      if (playSound === true) {
        playCardSound('slide');
      }
    }, timeout);
...
function playerTurn(result) {
  ...
  globalTimeout1= setTimeout(function(){{console.log('Timer 2 deleted');$standButton.click();clearTimeout(globalTimeout1);clearInterval(countTimerout);$('.counttimer_down').css('visibility','hidden');}},waitingTime);
  countTime=30;
  clearInterval(countTimerout);
  countTimerout=setInterval(function(){
  ...
  if (player.money >= player.bet && player.splitHand === null) {
    ...
    if(!$._data($('#double-button')[0], 'events')) {
      // var timer1= setTimeout(function(){leaveGame();clearTimeout(timer1);},waitingTime);
  ...
  if (!$._data($('#hit-button')[0], 'events')) {
    // var timer1= setTimeout(leaveGame,waitingTime);
...
function stand(result) {
 ...
 //      console.log('Card added in Stand');
 // $(`#player-hand-${player.splitHand}`).append(result);
...
socket.on('player split', function(data) {
 ...
 console.log('BeginNewCard:'+$newCard2);
  if (data.player.id === socket.id) {
    ...
    // console.log(data.player.displayTotal);
    $(`#player-hand-0`).append($newCard1);
   // console.log('BeginNewCard1:'+$newCard2);
    let i = 0;
    ...
    while (player.total[i] === 21) {
      ...
      socket.emit('player money',{id:player.id,price:player.money});
    ...
    if (player.splitHand < player.hand.length) {
       let $newCardTitle = ($('<div>', {'class': 'card flyin'}));
       $newCardTitle.css('background-image', `url('${newCard2.img}')`);
      if(player.splitHand>0)
      {
         $(`#player-hand-${player.splitHand}`).append($newCardTitle);
        console.log('append card in player split ');
         setTimeout(function() {
    if (data.player.id === socket.id) {
      $('#player-total p').html(`${player.displayTotal[1]}`);
    } else {
      $(`#${playerId}`).prev().find('.hand-player-total').html(data.player.displayTotal[1]);
    }
  }, 250);
      }
      $(`#player-hand-${player.splitHand}`).addClass('selected');
      playerTurn($newCard2);
            ...
            console.log('selected:Other hand');
            var timer1= setTimeout(function(){leaveGame();clearTimeout(timer1);clearInterval(countTimerout);$('.counttimer_down').css('visibility','hidden');},waitingTime);
            countTime=30;
            clearInterval(countTimerout);
countTimerout=setInterval(function(){
 ...
 if(data.player.splitHand>0)
 {
  ...
  setTimeout(function() {
    if (data.player.id === socket.id) {
      $('#player-total p').html(`${player.displayTotal[0]}`);
    } else {
      $(`#${playerId}`).prev().find('.hand-player-total').html(data.player.displayTotal[0]);
    }
  }, 250);
...
socket.on('whose turn', function(data) {
  ...
  if (data.player.id === socket.id) {
      ...
      setTimeout(function()
                  {
                    ...
                    let newCard2 = data.player.hand[1][1];
      let $newCard2 = ($('<div>', {'class': 'card flyin'}));
      $newCard2.css('background-image', `url('${newCard2.img}')`);

        $(`#player-hand-${player.splitHand}`).append($newCard2);
        console.log('append card in whoose turn');}
      },300
    ...
    if (data.player.splitHand !== null) {
      ...
      $(`#${data.player.id}-${data.player.splitHand}`).append($newCard2);
        console.log('append card in whoose turn');}
      },300
    ...
    if (data.player.id === socket.id) {
      ...
      $(`#${data.player.id}`).prev().find('.hand-player-total').html(data.player.displayTotal.join('<br/>'));
    }
  }, 250);
    }
  }
...
function hitMe(recipient, card) {
  ...
  $newCard.css('background-image', `url('${card.img}')`);
 globalTimeout1= setTimeout(function(){{console.log('Global Timeout 1 Start');$('#stand-button').click();clearTimeout(globalTimeout1);clearInterval(countTimerout);$('.counttimer_down').css('visibility','hidden');
}},waitingTime);
 countTime=30;
 clearTimeout(countTimerout);
countTimerout=setInterval(function(){
  ...
  if(recipient.splitHand === null) {
    ...
    if (recipient.id === socket.id) {
      ...
      $(`#player-total p`).text(player.displayTotal[player.splitHand]);
      ...
      $(`#${recipient.id}`).prev().find('.hand-player-total').html(recipient.displayTotal[recipient.splitHand]);
    }
  }
...
socket.on('new dealer card', function(data) {
  ...
  setTimeout(function() {
    ...
    $dealerFirstCard.css('background-image', `url(${data.dealerHiddenCard})`);
  }, 500);
...
socket.on('ChargeForPlayer',function(data){
  console.log('Price:');
  console.log(data);
  player.money=parseInt(data.price);
...
function endGame(dealerHiddenCard, dealerTotal, winStatus, message) {
  ...
  let $splitButton = $('#split-button');
  let $dealerFirstCard = $('#dealer-hand div:nth-child(1)');
  $dealerCount=$('#dealer-hand div');
  console.log($dealerCount.length);
  // console.log($dealerFirstCard.css('background-image'));
if($dealerCount.length<3){
  console.log('resultA');
  // console.log($dealerFirstCard.css('background-image'));
  // console.log(`url(${dealerHiddenCard})`);
  $dealerFirstCard.removeClass('flyin');
  $dealerFirstCard.addClass('loop');
  if (playSound === true) {
    playCardSound('flip');
  ...
  setTimeout(function() {
    if (winStatus) {
      ...
      if (winStatus === 'win') {
        ...
        winStatus.forEach((status, index) => {
          if (status === 'win') {
            ...
            $('#player-bet p').html('$0');
            $('#player-money p').text(`$${centify(player.money)}`);
            socket.emit('player money',{name:player.name,price:player.money});
          }
        })
      }
      ...
      clearInterval(countTimerout);
      $('.counttimer_down').css('visibility','hidden');
        console.log("globalTimeout1 End");
      ...
      var timer111= setTimeout(function(){
        // resetGame();
        leaveGame();
...
countTimerout=setInterval(function(){
  ...
  $('.counttimer_down').html(countTime+' Secs Remaining');    
      ...
      $standButton.one('click',function(){
        clearTimeout(timer111);
        console.log('Timer 4 End');
        $('body').children('.chipBtn').remove();
