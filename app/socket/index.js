	...
	var Card = function(suit,value,realValue) {
	  ...
	  function dealerTurn() {
		...
		while (dealer.total < 17) {
		  ...
		  setTimeout(function() {
			io.sockets.emit('new dealer card', {card: newCard,dealerHiddenCard: dealer.hand[0].img});
		  }, timeout);
	  ...
	  io.on('connection', function(socket) {
		...
		socket.on('player money',function(data){
			User.updatePrice(data.id,data.price,function(err,todo){
				if(!err)
				{
					console.log('______________________Success_________________________');
				}
			});
		})
		...
		socket.on('hit me', function() {
		  ...
		  players.forEach((player) => {
			if (player.id === socket.id) {
			  ...
			  console.log('server  hit new card');
			  if (player.total > 21 || player.total[player.splitHand] > 21) {
				...
				if (player.splitHand === null || player.splitHand + 1 >= player.hand.length) {
				  ...
				  console.log('server  hit new card1');

				  player.splitHand++;
				  // console.log('Black Jack');
				  // player.total[player.splitHand]=21;
		...
		socket.on('previewCharge',function(data){
			console.log(data);
		 User.findById(data.id,function(err,user){
			 console.log(user);
				if(user.chargePrice==='0')
				{
						console.log('0000000000000000');
				}
				else
				{
					console.log('UserChargePrice',user.chargePrice);
					socket.emit('ChargeForPlayer',{price:user.chargePrice});
					User.chargePrice(data,user.chargePrice,function(err,todo){
